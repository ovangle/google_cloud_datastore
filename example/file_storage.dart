...
class File extends Entity {
  ...
  static Future<File> fromIoFile(Datastore datastore, io.File ioFile, String descriptor) {
    ...
    return datastore.allocateKey("File").then((Key fileKey) {
      return ioFile.length().then((fileLength) {
        var file = new File._(fileKey, fileDescriptor: descriptor, sizeInBytes: fileLength);
  ...
  @constructKind
  File._(Key key, {String fileDescriptor, int sizeInBytes}) :
    super(key, {"descriptor" : fileDescriptor, "sizeInBytes" : sizeInBytes});
  ...
  Stream<FilePart> getParts(Datastore datastore) {
   ...
   * of the given file.
   */
  Future writeToFile(Datastore datastore, io.File ioFile) {
    ...
    return getParts(datastore).toList()
        .then((parts) {
          parts.sort();
...
class FilePart extends Entity implements Comparable<FilePart>{
  ...
  @constructKind
  FilePart._(Key key, {Uint8List content}) :
    super(key, {'file_content' : content});
